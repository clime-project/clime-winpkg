...
　`u+` または `U+` に続けて（16進表記の）Unicode コードポイントを入力し、Enter を
押下することで指定した Unicode 文字を入力することができます。たとえば `u+5b57` と
入力して Enter を押せば「字」という文字を入力することができます。
...
| F1          | この文書を表示                         |
| F9          | テーマの切り替え                       |
| Alt+F10     | ターゲットの実行モジュール名取得       |
| Alt+F12     | 最後に確定入力した文字列の「リコール」 |
...
| Ctrl+P, ↑  | 前の候補を選択                             |
| Ctrl+N, ↓  | 次の候補を選択                             |
| PageUp      | 候補を１ページ分上にスクロール            |
| PageDown    | 候補を１ページ分下にスクロール            |
